...
class TicketAndSalesRouteProxyTest {
    ...
    static void registerGatewayRoutes(DynamicPropertyRegistry registry) {
        try {
            ...
            throw new IllegalStateException("Failed to start ticket/sales gateway test backend", exception);
        }
        registry.add("USER_SERVICE_URI", () -> "http://localhost:65531");
        registry.add("EVENT_SERVICE_URI", () -> "http://localhost:65532");
        registry.add("BOOKING_SERVICE_URI", () -> "http://localhost:65533");
        registry.add("TICKET_SERVICE_URI", () -> backendBaseUrl);
        registry.add("SALES_SERVICE_URI", () -> backendBaseUrl);
    }

    @Test
    void proxiesTicketAndSalesRoutesThroughGateway() {
        webTestClient.get()
                .uri("/api/tickets/health")
                .header("Authorization", "Bearer " + testJwt())
                .exchange()
                .expectStatus().isOk()
                .expectBody(String.class)
                .value(body -> assertThat(body).isEqualTo("TICKET-OK"));

        webTestClient.get()
                .uri("/api/sales/health")
                .header("Authorization", "Bearer " + testJwt())
                .exchange()
                .expectStatus().isOk()
                .expectBody(String.class)
                .value(body -> assertThat(body).isEqualTo("SALES-OK"));
    }

    @Test
    void ticketAndSalesRoutesRequireJwt() {
        webTestClient.get()
                .uri("/api/tickets/health")
                .exchange()
    ...
    private static void ensureBackendStarted() throws IOException {
        ...
        backend.createContext("/api/tickets/health", new PlainTextHandler(200, "TICKET-OK"));
        backend.createContext("/api/sales/health", new PlainTextHandler(200, "SALES-OK"));
        backend.start();
